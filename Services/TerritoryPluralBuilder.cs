...
namespace NorthwindAPI.Services {
    ...
    public class TerritoryPluralBuilder : ITerritoryPluralBuilder {
        ...
        public TerritoryPluralBuilder(EntitiesContext context, IMapper mapper, ILoggerManager logger) {
            _entities = context;
            _mapper = mapper;
            _logger = logger;
        }
        ...
        private Expression<Func<TerritoryPlural, TerritoryPluralModel>>  ProjectToModel {
            get {
                return entity => _mapper.Map<TerritoryPluralModel>(entity);
            }
        }
        
        public IQueryable<TerritoryPluralModel> GetTerritoryPlurals() {
            return _entities.TerritoryPlurals.Select(ProjectToModel);
        }
        ...
        public async Task<BuilderResponse> GetTerritoryPlural_ByTerritoryID(string territoryID) {
          var query = await Search(_entities.TerritoryPlurals, x => x.TerritoryID == territoryID).Select(ProjectToModel)?.ToListAsync();
           ...
           return new BuilderResponse { RequestMessage = $"Record Not Found; TerritoryPlural with territoryID = '{territoryID}' doesn't exist." }; 
            }
        }
        
        public IQueryable<TerritoryPluralModel> GetTerritoryPlural_ByRegionID(int regionID) {

          var query = Search(_entities.TerritoryPlurals, x => x.RegionID == regionID).Select(ProjectToModel);
        ...
        public async Task<BuilderResponse> AddTerritoryPlural(TerritoryPluralModel model) {
            ...
            if (!matchRegionID.Any()) {
              return new BuilderResponse { RequestMessage = $"Foreign Key Violation; " + nameof(model.RegionID) + " '{model.RegionID}' doesn't exist in the system."}; 
            }

           try
           {
                 var entity = _mapper.Map<TerritoryPlural>(model);
        ...
        public async Task<BuilderResponse> UpdateTerritoryPlural(TerritoryPluralModel model) {
          ...
          var query = Search(_entities.TerritoryPlurals, x =>  x.TerritoryID == model.TerritoryID);
            if (!query.Any()) {
              return new BuilderResponse { RequestMessage = "Record Not Found; " + string.Format("TerritoryPlural with _territoryID = '{0}' doesn't exist.",model.TerritoryID)}; 
            }
            var matchRegionID = _entities.Regions.Where(x => x.RegionID.Equals(model.RegionID));
            if (!matchRegionID.Any()) {
              return new BuilderResponse { RequestMessage = "Foreign Key Violation; " + nameof(model.RegionID) + string.Format("RegionID = '{0}' doesn't exist in the system.", model.RegionID)}; 
            }
        ...
        public async Task<BuilderResponse> DeleteTerritoryPlural(string territoryID) {
          var query = Search(_entities.TerritoryPlurals, x => x.TerritoryID == territoryID);
            if (!query.Any()) {
              return new BuilderResponse { RequestMessage = "Record Not Found; " + string.Format("TerritoryPlural with _territoryID = '{0}' doesn't exist.",territoryID)}; 
            }
